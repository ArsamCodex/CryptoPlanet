...
def homePage():
    ...
    return render_template("index.html", content=current_rate, btc=each_crypto_id.upper(), price=current_rate,
                           symbol=cryotSymbol, ethName=cryptoSymbolEth, ethSymbol=cryptoIdEth.upper(),
                           ethPrice=curentRateEth, usdtName=cryptoIdUsdt.upper(), usdtSymbol=cryptoSymbolUsdt,
                           usdtPrice=curentRateUsdt
                           , bnbName=cryptoIdBnbt.upper(), bnbSymbol=cryptoSymbolBnb, bnbPrice=curentRateBnb
                           , time=datetime.utcnow(), info=current_rate, info2=curentRateEth, info3=curentRateUsdt,
                           info4=curentRateBnb)
...
def trending():
    ...
    trendingcoinsInformation = json.loads(requests.get('https://api.coingecko.com/api/v3/search/trending').content).get(
        'coins')
...
def identityVerification():
    app.config["UPLOAD_FOLDER"] = "static/f/"
